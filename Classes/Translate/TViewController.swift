...
class TViewController: JSQMessagesViewController {
    
    @IBOutlet weak var VoiceRec: UIButton!
    ...
    func downloadThumbnail(user: PFUser) -> Void{
        if let stringUrl = user[PF_USER_THUMBNAIL] as? String {
            AFDownload.start(stringUrl, complete: {(path: String!, error: NSError!, network: Bool) in
            if error == nil {
